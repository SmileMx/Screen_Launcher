...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener  {
    ...
    private ValueAnimator mScrollIndicatorBgAnimator;
    private View mScrollIndicator;
    private View mScrollIndicatorBg;	//页码指示背景
    ...
    protected int getScaledMeasuredWidth(View child) {
        ...
        //init mMinimumWidth add by xiangss 2013-09-18
        if(mMinimumWidth == 0){
        	mMinimumWidth = measuredWidth;
        }
    ...
    protected View getScrollingIndicator() {
        ...
        if (mHasScrollIndicator && mScrollIndicator == null) {
            ...
            if (parent != null) {
                ...
                mScrollIndicatorBg = (View) (parent.findViewById(R.id.paged_view_indicator_bg));
                mHasScrollIndicator = mScrollIndicator != null;
                if (mHasScrollIndicator) {
                    mScrollIndicator.setVisibility(View.VISIBLE);
                }
                
                //添加页码指示背景  add by xiangss 20130831
                if(mScrollIndicatorBg != null){
                	mScrollIndicatorBg.setVisibility(View.VISIBLE);
                }
            }
        }
    ...
    protected void flashScrollingIndicator(boolean animated) {//3
        removeCallbacks(hideScrollingIndicatorRunnable);
        showScrollingIndicator(!animated);
        postDelayed(hideScrollingIndicatorRunnable, sScrollIndicatorFlashDuration);
    }

    protected void showScrollingIndicator(boolean immediately) {//7
        ...
        if (mScrollIndicatorBg != null) {
            // Fade the indicator in
            mScrollIndicatorBg.setVisibility(View.VISIBLE);
            cancelScrollingIndicatorBgAnimations();
            if (immediately || mScrollingPaused) {
                mScrollIndicatorBg.setAlpha(1f);
            } else {
            	mScrollIndicatorBgAnimator = LauncherAnimUtils.ofFloat(mScrollIndicatorBg, "alpha", 1f);
            	mScrollIndicatorBgAnimator.setDuration(sScrollIndicatorFadeInDuration);
            	mScrollIndicatorBgAnimator.start();
            }
        }
        
    }

    protected void cancelScrollingIndicatorAnimations() {//7
    ...
    //添加页码指示背景  add by xiangss 20130831
    protected void cancelScrollingIndicatorBgAnimations() {
        if (mScrollIndicatorBgAnimator != null) {
        	mScrollIndicatorBgAnimator.cancel();
        }
    }
    
    protected void hideScrollingIndicator(boolean immediately) {//15
        if (getChildCount() < 1) return;
        if (!isScrollingIndicatorEnabled()) return;
        ...
        if (mScrollIndicatorBg != null) {
            ...
            cancelScrollingIndicatorBgAnimations();
            if (immediately || mScrollingPaused) {
                mScrollIndicatorBg.setVisibility(View.INVISIBLE);
                mScrollIndicatorBg.setAlpha(0f);
            } else {
            	mScrollIndicatorBgAnimator = LauncherAnimUtils.ofFloat(mScrollIndicatorBg, "alpha", 0f);
            	mScrollIndicatorBgAnimator.setDuration(sScrollIndicatorFadeOutDuration);
            	mScrollIndicatorBgAnimator.addListener(new AnimatorListenerAdapter() {
                    private boolean cancelledBg = false;
                    @Override
                    public void onAnimationCancel(android.animation.Animator animation) {
                    	cancelledBg = true;
                    }
